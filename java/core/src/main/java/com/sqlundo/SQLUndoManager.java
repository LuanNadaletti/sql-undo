...
import com.sqlundo.dto.RevertedQuery;
import com.sqlundo.enums.QueryReverserType;
...
public class SQLUndoManager {
    ...
    public static List<RevertedQuery> undo(String scriptText) {
        List<RevertedQuery> queries = new ArrayList<>();
        for (Query query : parseQueries(scriptText)) {
            queries.add(new RevertedQuery(query.toString(), reverseQuery(query)));
        }
